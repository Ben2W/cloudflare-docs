...
When your site has forms that accept user input, you need to decide how to verify that visitors are human, how aggressively to limit repeated submissions, and which request patterns to block outright. Contact forms, registration pages, payment forms, and comment sections are common targets for automated abuse, including spam, fake account creation, and card testing. This guide covers four stages: add human verification with [Turnstile](/turnstile/), rate limit form submission endpoints, deploy [Application Security](/waf/) rules for known abuse patterns, and monitor for signs of ongoing abuse. The core workflow uses features available on all plans. Pro and Business plan features are called out where they add value.

:::note
Most procedures in this guide are configured per domain. Select your domain in the Cloudflare dashboard before starting. Turnstile is the exception: widgets are configured at the account level.
:::

## Add Turnstile to your forms

Form submissions from automated scripts are a common source of spam and abuse. Turnstile verifies visitors are human without visible challenges. This guide uses Managed mode, which automatically chooses between a non-interactive or checkbox challenge based on visitor risk level. For other widget modes, refer to [Widget types](/turnstile/concepts/widget/). {/* Source: /turnstile/get-started/ — Turnstile product description; /turnstile/concepts/widget/ — Managed mode behavior */}
...
{/* Source: /turnstile/get-started/widget-management/dashboard/ — existing partial covers full procedure */}
...
{/* Source: /turnstile/tutorials/integrating-turnstile-waf-and-bot-management/ — client-side integration pattern */}
...
Server-side validation is required. The client-side widget alone does not protect your forms because attackers can submit directly to your form endpoint. Tokens expire after 300 seconds (five minutes) and can only be validated once. {/* Source: /turnstile/get-started/server-side-validation/ — mandatory validation requirement and token characteristics */}
...
{/* Source: /turnstile/get-started/server-side-validation/ — JavaScript validation example using JSON format */}
...
Add Turnstile to any form that accepts user input and triggers a server-side action, including contact forms, registration pages, payment flows, and comment sections.
...
{/* Source: /waf/rate-limiting-rules/find-rate-limit/ — scope and rate analysis workflow */}
...
Create a rule that limits how many times a single IP address can submit to your form endpoint within a given period. The following example uses a contact form at `/contact` with a threshold of five requests per 10 seconds. Adjust the path, threshold, and period for your site.

<Render file="create-rate-limiting-rule" product="waf" params={{ ruleName: "Rate limit contact form submissions", expression: '(http.request.uri.path eq "/contact" and http.request.method eq "POST")', expressionHelp: 'Replace "/contact" with your form endpoint path.' }} />

{/* Source: /waf/rate-limiting-rules/create-zone-dashboard/ — dashboard rule creation procedure; shared partial covers plan-aware steps */}
...
:::note[Pro plans and above]
Custom responses for rate limiting blocks are available on Pro plans and above.
:::

Instead of showing the default Cloudflare error page when a rate limit is reached, return a message that tells the visitor what happened and when they can retry. For details on configuring a custom response body, refer to [Create a rate limiting rule in the dashboard](/waf/rate-limiting-rules/create-zone-dashboard/#configure-a-custom-response-for-blocked-requests).
...
### Create a custom rule for spam patterns

Create a custom rule that challenges POST requests to your form endpoints when the request contains patterns common in spam submissions.
...
3. Enter a descriptive name, such as "Challenge spam form submissions".
...
{/* Source: /waf/custom-rules/create-dashboard/ — custom rule creation procedure (new dashboard) */}
...
{/* Source: /bots/get-started/bot-fight-mode/ — existing partial covers enable procedure */}

Bot Fight Mode protects your entire domain without endpoint restrictions. You cannot create exceptions using custom rules to bypass Bot Fight Mode. If you need to exempt specific paths or allow specific bots (such as monitoring services or partner APIs), upgrade to Super Bot Fight Mode on Pro or above. {/* Source: /bots/get-started/bot-fight-mode/ — limitation: Skip action does not work with BFM */}
...
{/* Source: /waf/analytics/security-events/ — dashboard location and event details */}

If you see a high challenge solve rate on a specific rule, the rule may be matching legitimate traffic. Consider narrowing the expression or switching to a less aggressive action.
...
If a third-party script is injected into your form page, it can exfiltrate submitted data, including payment information. Client-Side Security monitors third-party scripts on your pages for changes and potential supply chain attacks. {/* Source: /client-side-security/ — product overview */}
...
{/* Source: /client-side-security/get-started/ — enable monitoring procedure (new dashboard) */}
...
- [Integrate Turnstile, WAF, and Bot Management](/turnstile/tutorials/integrating-turnstile-waf-and-bot-management/) — tutorial combining all three products for form protection
...
- [Bot Fight Mode](/bots/get-started/bot-fight-mode/) — free bot protection for all plans
